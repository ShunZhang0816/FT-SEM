...
for( k in 1:6){
  ...
  for( i in 1:(nSum%/%nCircle + 1)){
    ...
    result <- foreach (j = 7:nsnp, .combine = rbind,
                       ...
                       .errorhandling = "pass") %dopar%{
                         ...
                         # 转换数据用于分析(不含fsiq)
                         # data_sub          <- family_unique[,c(7,10:13,13 + k,19:28,# SNP、协变量、结局、十个PC (子代)
                         #                                       34,37:40,40 + k, # SNP、协变量、结局（父亲）
                         #                                       61,64:67,67 + k # SNP、协变量、结局（母亲）
                         #                                       ),with = F]
                         
                         # 转换数据用于分析(含fsiq)
                         data_sub          <- family_unique[,c(7,10:13,13 + k,20:29,# SNP、协变量、结局、十个PC (子代)
                                                               35,38:41,41 + k, # SNP、协变量、结局（父亲）
                                                               63,66:69,69 + k # SNP、协变量、结局（母亲）
                         ),with = F]
                         
                         data_sub_2        <- independent_data[,c(7,10:13,13 + k,20:29),with = F]# SNP、协变量、结局、十个PC (子代)
  ...
  # 输出注释后的结果文件
  file_path_2 <- paste0("G:/phs000620/MCTFR_clean/Output_end_20241121/", result_name[k], "_full.csv")
